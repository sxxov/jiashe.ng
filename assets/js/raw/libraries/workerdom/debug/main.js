 ...
 * @param type
 */
var isUserVisibleMutation = type => {
  switch (type) {
    case 4
    /* EVENT_SUBSCRIPTION */
    :
 ...
 */
var nodeListEach = (list, callback) => Array.prototype.forEach.call(list, callback);
...
var EventSubscriptionProcessor = (strings, nodeContext, workerContext, objectContext, config) => {
  ...
  var eventHandler = (index, preventDefault) => event => {
    if (preventDefault) {
  ...
  var processListenerChange = (target, addEvent, mutations, iterator) => {
...
class MutatorProcessor {
  ...
  constructor(stringContext, nodeContext, workerContext, config, objectContext) {
    ...
    this.syncFlush = function (allowVisibleMutations) {
      if (allowVisibleMutations === void 0) {
        allowVisibleMutations = true;
      }

      var disallowedMutations = [];

      _this.mutationQueue.forEach(mutationArray => {
